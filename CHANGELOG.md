...
- **Masthead: structured "What changed", folded window chips, no
  ...
  clause | section link) instead of prose paragraphs; the compared-windows
  strip keeps its DB-time chart and folds the dated chips behind a
  one-line summary (`<details class="windows-more">`, Current chip +
  "vs N prior windows, every step, back to <date>"); the **Detailed**
  view is renamed **Full** (`body.full`, `#mode-full`, `.full-only`,
  hash `!v=f`, localStorage value `full`); the **Application only**
  filter is removed entirely (button, CSS, `awr:appfilter`, hash `a`,
  06's `sys` series filter) -- the `data-sys` tags stay as metadata.
  `docs/metric_policy.html` (generated by `docs/gen_policy_doc.py` from
  `sql/lib/metric_policy.plsql`) documents every metric's direction and
  floors, when it fires and when it does not.
- **Normal / Full views (phase 7).** The report now opens in a
  **Normal** view -- masthead, Headline metrics, the Findings movers,
  ...
  and a **Full** view that is the whole report. A two-button switch
  in the rail foot (`#mode-normal` / `#mode-full`, `body.normal` /
  `body.full`, persisted as localStorage `awr-mode`, shared in the
  hash as `!v=f`) replaces the **Triage mode** and **Essential rows**
  toggles, which are gone (their `data-imp` row tags stay, unused by
  CSS). `.full-only` marks Full-only content inside a kept section
  (07's per-domain tables, 06's per-SQL pool, 16's hour table); the rail
  hides links to hidden sections behind a "+ N more in Full" line, a
  `.mode-note` at the end of `<main>` names them, `revealHash()` flips to
  Full when a link targets hidden content, and `setMode()` resizes
