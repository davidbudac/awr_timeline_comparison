...
BEGIN
    ...
    FOR s IN (
        ...
        -- in every prior window, derived from the same week_vals CSV used
        -- for the chart series above.
        DECLARE
    ...
    FOR tc IN (
        ...
        v_row := '<tr>'
            ...
            || '<td class="num" data-w="0"' || fmt_num_title(tc.cur_val) || '><b>'
                || fmt_num(tc.cur_val)
    ...
    IF v_dim_label.COUNT > 0 THEN
        ...
        DBMS_OUTPUT.PUT_LINE('  var lastMode="files", hiSlot=null;');
        DBMS_OUTPUT.PUT_LINE('  function render(mode){');
        DBMS_OUTPUT.PUT_LINE('    if(mode) lastMode=mode;');
        DBMS_OUTPUT.PUT_LINE('    var rows=(lastMode==="ftypes"?d.ftypes:d.files)||[];');
