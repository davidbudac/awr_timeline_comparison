...
BEGIN
    ...
    -- The codewrap/copy-btn pair is markup only; the chrome agent wires the
    -- click handler that copies #awr-report-sql's text (C4).
    DBMS_OUTPUT.PUT_LINE('<div class="codewrap" style="position:relative">');
    DBMS_OUTPUT.PUT_LINE('<button type="button" class="copy-btn" data-copy="#awr-report-sql">Copy</button>');
    DBMS_OUTPUT.PUT_LINE('<pre id="awr-report-sql" class="sql">'
