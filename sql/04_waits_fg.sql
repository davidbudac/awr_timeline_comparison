...
-- Each group is rendered twice: total time waited (s) and average time
-- per wait (ms = time_waited_us / total_waits / 1000).
...
DECLARE
    ...
    TYPE t_class_rec IS RECORD (
        wait_class    VARCHAR2(64),
        cur_us        NUMBER,
        mu_us         NUMBER,
        sd_us         NUMBER,
        n_us          NUMBER,
        week_vals     VARCHAR2(4000)
    );
...
BEGIN
    ...
    -- ===============================================================
    -- Top-N events: collect once, render two tables (s) and (ms/wait)
    -- ===============================================================
    WITH
    @@sql/lib/windows_cte.sql
    ,
    ...
    SELECT event_name,
           ...
           -- Units are passed through raw -- z and pct are unit-invariant.
    ...
    -- ===============================================================
    -- Wait-class rollup: collect once, render two tables
    -- ===============================================================
    WITH
    @@sql/lib/windows_cte.sql
    ,
    ...
    grid AS (
        SELECT c.wait_class, w.week_offset, cd.time_waited_us, cd.total_waits
        FROM   classes c
        CROSS JOIN all_weeks w
        LEFT JOIN class_deltas cd
               ON cd.wait_class = c.wait_class AND cd.week_offset = w.week_offset
