...
BEGIN
    ...
    grid AS (
        -- Measures come from `deltas` (FULL history), rnk from top_n_events --
        -- so an event that ranked below top_n in a prior window still feeds the
        -- mu/sd/n baseline instead of contributing an n=0 "insufficient
        -- history" gap for a newly-spiking event (F1). Mirrors 04_waits_fg.
