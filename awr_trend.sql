...
--   profile_days 0 (default = off) or the number of prior days for the
...
-- -------------------------------------------------------------------
-- Sections are included in VISUAL order (the rail's order: Triage,
-- Workload, SQL, Storage & config) so the DOM order, the keyboard tab
-- order and the screen-reader order all match what the page shows --
-- the stylesheet no longer re-sorts sections with flex order:.  Every
-- section is an independent recompute, so the order is free; 17 must stay
-- last (it relocates itself into the masthead) and 18 precedes it.
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('Generated by awr_trend.sql v' || '~awr_version'
        || ' &mdash; run ' || '~run_id'
