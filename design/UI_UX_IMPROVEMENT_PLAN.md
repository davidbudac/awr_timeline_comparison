...
### Phase 8 — Masthead clean-up, Full view, no app filter, policy reference — DONE 2026-09-22

Requested after phase 7 from the sample report: "a lot of text" in the
"What changed" block, the dated window list not wanted as is, rename
Detailed to Full, drop the Application-only filter, and a travelling
piece of documentation for the per-metric policy.

- **8.1 "What changed"** is a structured list: one row per rule with
  label, headline number (ratio or percent), from -> to, one short why
  clause and a section link (`add_item()` in `17_narrative.sql`,
  `.narr-list` grid in `_style.sql`, twin re-ported). No prose sentences.
- **8.2 Compared windows** keeps the DB-time chart; the dated chips fold
  behind a `<details>` summary (Current chip + "vs N prior windows, every
  step, back to <date>" + show/hide). The Current chip inside the summary
  still highlights without toggling the fold.
- **8.3 Full view** replaces "Detailed" everywhere (`body.full`,
  `#mode-full`, `.full-only`, hash `!v=f`, localStorage `full`). The
  Application-only filter is gone (button, CSS, JS, `awr:appfilter`, 06's
  `sys` series filter, hash `a`); `data-sys` tags stay as metadata.
- **8.5 Fleet day-profile band** (`sql/fleet/06_day_profile.sql`) now
  re-buckets each cell through `policy_bucket()`, the last fleet scorer on
  the old rule.
- **8.4 `docs/metric_policy.html`**, generated by `docs/gen_policy_doc.py`
  from the policy file: the rule step by step with a worked example,
  direction / bucket legends, one table row per load statistic, system
  metric, wait class and event override (unit, family, direction, min %,
  floor, fires when, does not fire when, a numeric example and the
  policy line's own note), the SQL / SEG / FILE defaults, families and
  twins, and how to edit. Linked from `docs/index.html` and the README.
